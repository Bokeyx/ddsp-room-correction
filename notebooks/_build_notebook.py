...
md(
    "## 8b. Full-rate top octave: Aachen AIR (48 kHz)\n"
    "\n"
    "The MIT mirror above is 16 kHz, so its correction stops just under 8 kHz. To check the\n"
    "**top octave (8–20 kHz)** we bring in a second, independent measured set: the **Aachen AIR\n"
    "database** (RWTH, 48 kHz). Different rooms, different microphones — so this is a fresh full-rate\n"
    "opinion, not a re-run of the MIT numbers. We keep the listening-type rooms (booth / office /\n"
    "meeting / lecture) and drop the band-limited phone recordings and the extreme spaces (stairways,\n"
    "the aula_carolina church hall). The data is gitignored — unzip the AIR `.mat` files under\n"
    "`data/air_database_release_1_4/`."
)

code(
    "from src.datasets import list_air_rirs, load_air_mat\n"
    "from src.pipeline import correct, smoothed_sigma\n"
    "\n"
    "AIR_DIR = '../data/air_database_release_1_4/AIR_1_4'\n"
    "AIR_ROOMS = ('booth', 'office', 'meeting', 'lecture')\n"
    "PER_ROOM = 2       # cap so the 48 kHz loop stays tractable\n"
    "AIR_ITERS = 120    # DDSP converges well before this at 48 kHz\n"
    "TOP = (8000.0, 20000.0)  # the octave the 16 kHz mirror could not reach\n"
    "try:\n"
    "    air_paths = list_air_rirs(AIR_DIR)\n"
    "except FileNotFoundError:\n"
    "    air_paths = []\n"
    "    print('Aachen AIR not found. Put the .mat files under data/air_database_release_1_4/.')\n"
    "\n"
    "curated = []\n"
    "for room in AIR_ROOMS:\n"
    "    hits = [p for p in air_paths if room in p.lower()]\n"
    "    curated += hits[:PER_ROOM]\n"
    "\n"
    "air_rows = []\n"
    "for p in curated:\n"
    "    h, fs, room = load_air_mat(p)\n"
    "    air_rows.append((room, evaluate_rir(h, fs, n_filters=NF, ddsp_iters=AIR_ITERS)))\n"
    "print(f'evaluated {len(air_rows)} full-rate rooms at {fs if air_rows else 0} Hz')"
)
...
code(
    ...
    "    methods = ['before', 'classic', 'ddsp', 'fir']\n"
    "    colors = ['tab:gray', 'tab:green', 'tab:red', 'tab:blue']\n"
    "    data = {m: np.array([r[m] for _, r in air_rows]) for m in methods}\n"
    "    means = [data[m].mean() for m in methods]\n"
    "\n"
    "    # detailed top-octave view on the first room: the 8-20 kHz region the\n"
    "    # 16 kHz mirror could never correct, before vs DDSP.\n"
    "    h0, fs0, room0 = load_air_mat(curated[0])\n"
    "    f0, r0 = frequency_response(h0, fs0)\n"
    "    d0, _ = correct(r0, flat_target(f0), f0, fs0, method='ddsp', n_filters=NF, ddsp_iters=AIR_ITERS)\n"
    "    top_before = smoothed_sigma(r0, f0, *TOP)\n"
    "    top_after = smoothed_sigma(d0, f0, *TOP)\n"
    "\n"
    "    fig, (ax1, ax2) = plt.subplots(1, 2, figsize=(13, 4))\n"
    "    bars = ax1.bar(methods, means, color=colors)\n"
    "    for b, m in zip(bars, means):\n"
    "        ax1.text(b.get_x()+b.get_width()/2, m, f'{m:.2f}', ha='center', va='bottom')\n"
    "    ax1.set(title=f'Aachen AIR 48 kHz: sigma over {len(air_rows)} rooms (full 20-20k band)',\n"
    "            ylabel='sigma [dB] (lower = better)')\n"
    "    ax2.semilogx(f0, fractional_octave_smooth(f0, r0), color='tab:gray', lw=1.5,\n"
    "                 label=f'before (top-oct sigma={top_before:.2f})')\n"
    "    ax2.semilogx(f0, fractional_octave_smooth(f0, d0), color='tab:red', lw=1.8,\n"
    "                 label=f'DDSP (top-oct sigma={top_after:.2f})')\n"
    "    ax2.axvspan(8000, 20000, color='orange', alpha=0.12, label='top octave (8-20 kHz)')\n"
    "    ax2.axhline(0, color='k', lw=0.8, ls='--', alpha=0.5)\n"
    "    ax2.legend(); ax2.set(xlim=(20, 20000), xlabel='frequency [Hz]', ylabel='magnitude [dB]',\n"
    "                          title=f'{room0}: the octave 16 kHz data could not reach')\n"
    "    plt.tight_layout(); plt.savefig('../assets/10_air_fullrate.png', dpi=110); plt.show()\n"
    "\n"
    "    for m in methods:\n"
    "        print(f'{m:8s} sigma = {data[m].mean():.3f} +/- {data[m].std():.3f}')\n"
    "    print(f'\\nTop octave (8-20 kHz) on {room0}: before {top_before:.2f} -> DDSP {top_after:.2f} dB —\\n'\n"
    "          f'the band the 16 kHz MIT mirror never touched is now corrected at full rate.')\n"
    "else:\n"
    "    print('skipped — no AIR data on disk')"
)
